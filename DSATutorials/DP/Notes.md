...
1. Partition DP:
   ...
   - Structure: index (start) + a for loop providing range of partitions between index and i.
   - We try all possible partition points and pick the optimal one.
   - Problems: Partition Array for Max Sum, Palindrome Partitioning II.

2. MCM (Matrix Chain Multiplication):
   - Objective: break problem at different places by applying K cuts between i and j.
   - Structure: two pointers (i, j) + a loop for cut position k between i and j.
   - At each cut, solve left and right independently, combine results.
   - Problems: MCM, Burst Balloons, Min Cost to Cut Stick, Egg Drop, Evaluate Expression to True.

3. LIS (Longest Increasing Subsequence):
   - Golden Rule: Every element is a LIS of length 1 by itself.
   - Structure: single index + variable tracking the previous index (or previous element).
   - Two choices: take or not take current element based on comparison with previous.
   - Optimization: O(nlogn) using patience sorting (binary search on tails array).
   - Problems: LIS, Longest Divisible Subset, Longest String Chain, Number of LIS.

4. Strings (LCS-based):
   - Core pattern: two indices (i, j) for two strings.
...
6. 2D Grid:
   ...
   - Mostly down and right directions (sometimes all 4 for min-cost problems).
   - Start from (0,0) or (n-1, m-1) and work towards opposite corner.
   - For 2-player problems (Cherry Pick): move both simultaneously using 3D DP.
   - Problems: Unique Paths, Min Path Sum, Triangle, Cherry Pick, Dungeon, Frog Jump.

7. 1D Array:
   - Simplest DP pattern: 1 jump or 2 jumps (Fibonacci-like recurrence).
   - dp[i] depends on dp[i-1] and/or dp[i-2] (sometimes dp[i-k]).
...
10. Identifying DP vs Greedy vs Backtracking:
    ...
    - "Print all solutions" → Backtracking (DP can't enumerate).
    - "Locally optimal = globally optimal" → Greedy.

11. State Definition Tips:
    - Ask: "What information do I need to make the next decision?"
    - Common states: index, remaining capacity, previous element, boolean flags.
    - If states are too many → look for ways to reduce dimensions.

12. Space Optimization Patterns:
    - If dp[i] only depends on dp[i-1] row → use two 1D arrays (prev, curr).
...
14. 1D DP: O(n) time, O(n) or O(1) space.
    ...
    LCS/Strings: O(n*m) time, O(n*m) or O(m) space.
    LIS: O(n^2) naive, O(nlogn) optimized.
    MCM/Partition: O(n^3) time, O(n^2) space.
    Subset/Knapsack: O(n*target) time and space.
